    ...
    $(".lobster-ex").each(function() {
        ...
        let exerciseSpec = {
          starterCode: dedent`
            ...
            //           the given item.
            bool bst_contains(Node *node, int item) {
              if (/* base case */) {
                
              } else if (/* equal to current datum */) {
                
              } else if (/* less than current datum */) {
                
              } else {
                /* more than current datum */
              }
            }
            ...
            int main() {
              ...
              assert(bst_contains(&n6, 7));
              assert(bst_contains(&n6, 2));
              assert(bst_contains(&n6, 10));
              assert(!bst_contains(nullptr, 2));
              assert(!bst_contains(&n9, 2));
            }
          `,
          checkpoints: [
